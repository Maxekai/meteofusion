...
class WeatherApiTestCase(unittest.TestCase):
    ...
    def test_get_forecast_routes_to_visual_crossing_provider(self) -> None:
        async def fake_obtain_visual_crossing_forecast(
            latitude: float,
            longitude: float,
            days: int,
        ) -> ProviderForecast:
        ...
        with patch(
            "app.api.weather.obtain_visual_crossing_forecast",
            new=fake_obtain_visual_crossing_forecast,
        ):
            response = client.get(
                "/api/weather/visual_crossing/forecast",
                params={
                    "latitude": BARCELONA_LATITUDE,
                    "longitude": BARCELONA_LONGITUDE,
                    "days": 7,
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["provider"], "visual_crossing")
